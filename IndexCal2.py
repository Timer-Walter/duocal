...
def indexCal(closed,opened,highed,lowed,amounted):
    ...
    ma40 = talib.SMA(closed, timeperiod=40)
    ma60 = talib.SMA(closed, timeperiod=60)
    ma90 = talib.SMA(closed, timeperiod=90)
    ma120 = talib.SMA(closed, timeperiod=120)
    ma150 = talib.SMA(closed, timeperiod=150)
    ma180 = talib.SMA(closed, timeperiod=180)
    ...
    maCal_list=[[ma5,ma10,50],[ma5,ma20,100],[ma5,ma30,150],[ma5,ma40,200],
                [ma5,ma60,300],[ma5,ma90,450],[ma5,ma120,600]]
    ...
    for [m1,m2,n] in maCal_list:
        if m1[-1] > m2[-1] and m1[-2] < m2[-2]:
            ...
            maUp_list.append([n,kCount])

        if (n==100 or n==150 or n==300) and m1[-1] < m2[-1] and m1[-2] > m2[-2]:
    ...
    if kUp==0:
        amountUp = amounted[-1]/(sum(amounted[-6:-1])/5)
        yangRate = (closed[-1]-opened[-1])/ (highed[-1] - lowed[-1])
    else:
        amountDown = amounted[-1]/(sum(amounted[-6:-1])/5)
    if closed[-2]>opened[-2]:preKamountUp= amounted[-2]/(sum(amounted[-7:-2])/5)

    for i in range(1,6):
        if closed[-i]>opened[-i]:
            yangKCount +=1
        else:
            break
    ...
    if macd[-1] < signal[-1] and macd[-2] > signal[-2]:
        for i in range(1,6):
            ...
            if highPointLong>0.5 and highPointLongRate>0.6 and rsi[-i]>70:
                macdSell = 1
                break


    return [maUp_list,maDown_list,amountUp,preKamountUp,
            amountDown,yangRate,yangKCount,rsi[-2],rsi[-1],lowHold,macdSell]
